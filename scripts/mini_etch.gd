...
@onready var dust_particles_emitter
@onready var etching_ray
@onready var wafer_boundary
@onready var burnt_edges
@onready var etch_timer: Timer
...
func etch_area():
	...
	if etching_ray.visible:
		...
		var collision = wafer_boundary.get_rect().intersects(etching_ray.get_rect())
		if collision:
